...
export default function OxygenDetailPage() {
  ...
  const oxygenData = useQuery(api.oceanElements.getByTimeRange, {
    startTime,
    endTime,
  });
  ...
  const processChartData = () => {
    ...
    const sortedData = [...oxygenData].sort(
    ...
    sortedData.forEach((item) => {
      if (!item.dissolvedOxygen) return;
  ...
  const chartData = useMemo(() => processChartData(), [oxygenData]);

  // Memoize the getLatestOxygen function to prevent it from being recreated on each render
  const getLatestOxygen = useMemo(() => {
    return (deviceId: string) => {
      if (!oxygenData) return "--";

      const deviceData = oxygenData
        .filter(
          (item) =>
            item.deviceId === deviceId && item.dissolvedOxygen !== undefined
        )
        .sort((a, b) => b.timestamp - a.timestamp);

      if (deviceData.length === 0) return "--";

      const dissolvedOxygen = deviceData[0]?.dissolvedOxygen;
      if (dissolvedOxygen === undefined) return "--";

      return `${dissolvedOxygen.toFixed(1)} mg/L`;
    };
  }, [oxygenData]);
    ...
    <main className="flex min-h-screen flex-col p-6 bg-gray-50">
      <div className="mb-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-2xl font-bold">溶解氧详细分析</h1>
            <p className="text-gray-500">查看和分析海水溶解氧历史数据</p>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="p-4 bg-white rounded-lg shadow-sm border">
          <h3 className="text-sm font-medium text-gray-500">平均溶解氧</h3>
        ...
        <div className="p-4 bg-white rounded-lg shadow-sm border">
          <h3 className="text-sm font-medium text-gray-500">最高溶解氧</h3>
          <p className="text-2xl font-bold mt-1">
        ...
        <div className="p-4 bg-white rounded-lg shadow-sm border">
          <h3 className="text-sm font-medium text-gray-500">最低溶解氧</h3>
          <p className="text-2xl font-bold mt-1">
